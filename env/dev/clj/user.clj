(ns user
  ...
  (:require
    ...
    [prismofeverything.mirror.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (prismofeverything.mirror.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (prismofeverything.mirror.config/system-config {:profile :test})
                                  (ig/prep)))))
